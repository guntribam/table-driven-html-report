...
async function minifyHtml(props: any): Promise<string> {
    try {
        const htmlnano = await import('htmlnano');
        await import('cssnano')
        await import('postcss')
        await import('terser')
        return (await htmlnano.process(template(props), {
...
export function htmlReporter({ projectName = '-/-', minify = false, directory = 'report', reportName = 'testsReport' }: TestDrivenHtmlReporterConfig = {}) {
    ...
    return (_: any, emitter: any) => {
        ...
        emitter.on('runner:end', async () => {
            ...
            await fs.promises.writeFile(filePath, minify ? (await minifyHtml(props)) : template(props))
            if (fs.existsSync(path.join(dirPath, 'template.js'))) {
                console.log("templatepath", path.join(dirPath, 'template.js'))
                try {
                    const { template } = await import(path.join(dirPath, 'template.js'))
                    await fs.promises.writeFile(path.join(dirPath, 'template.js'), template.toString())
                } catch (error) {
...
const template = (_: TemplateProps) => `
  <!DOCTYPE html>
  <html lang="en">
