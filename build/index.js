...
function minifyHtml(props) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            return (yield htmlnano.process(template(props), {
...
function htmlReporter({ projectName = '-/-', minify = false, directory = 'report', reportName = 'testsReport' } = {}) {
    ...
    return (_, emitter) => {
        ...
        emitter.on('runner:end', () => __awaiter(this, void 0, void 0, function* () {
            ...
            const props = Object.assign(Object.assign({}, indexes), { totalTime: suiteTime, timestamp: new Date().toISOString().slice(0, new Date().toISOString().length - 8).replace('T', ' - '), successRate,
                failedRate,
                successRadius,
                failedRadius,
                isSuccessPrimary,
            ...
            yield fs_1.default.promises.writeFile(filePath, minify ? (yield minifyHtml(props)) : template(props));
            if (fs_1.default.existsSync(path_1.default.join(dirPath, 'template.js'))) {
                console.log("templatepath", path_1.default.join(dirPath, 'template.js'));
                try {
                    const { template } = yield Promise.resolve().then(() => __importStar(require(path_1.default.join(dirPath, 'template.js'))));
                    yield fs_1.default.promises.writeFile(path_1.default.join(dirPath, 'template.js'), template.toString());
                }
                catch (error) {
                    throw new Error("there is an error on your template.js file: " + error.message);
                }
            }
            else {
                yield fs_1.default.promises.writeFile(path_1.default.join(dirPath, 'template.js'), template.toString());
      ...
      <div class="test-chart">
        <div class="test-chart-pie">
          <svg height="150" width="150" viewBox="0 0 50 50">
            ...
            <circle r="10.5" cx="25" cy="25" fill="transparent" stroke="${_.isSuccessPrimary ? '#E73E48' : '#27CAA9'}" stroke-width="21"
              stroke-dasharray="calc(${_.isSuccessPrimary ? _.failedRadius : _.successRadius} * (3.1416 * 21) / 100) calc(3.1416 * 21)"
              transform="rotate(-90) translate(-50)" />
            <circle r="10.5" cx="25" cy="25" fill="transparent" stroke="${_.isSuccessPrimary ? '#27CAA9' : '#E73E48'}" stroke-width="21"
              stroke-dasharray="calc(${_.isSuccessPrimary ? _.successRadius : _.failedRadius} * (3.1416 * 21) / 100) calc(3.1416 * 21)"
              transform="rotate(-90) translate(-50)" />
          </svg>
        </div>
            ...
            <div class="test-chart-metadata">
              <ul>
                ...
                <li>
                  ...
                  <span>${(_.failedRate * 100).toFixed(2)}%</span>
                </li>
                <li>
